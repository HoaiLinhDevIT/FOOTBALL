...
export class ChangePhoneComponent implements OnInit {
    ...
    this.customerService
      ...
      .subscribe((res: HttpClientCountryResponse) => {
        ...
        let coutryIdDefault = this.listCountry.find((l) => {
          return l.countryName === this.data;
        })?.countryId;
  ...
  public stepPhone() {
    if (!this.validatorPhone())  {
      ...
      this.customerService.verifyPhone(
        {
        ...
        .subscribe({
          ...
          error: (err: HttpErrorResponse) => {
            ...
            if (err.error.meta.code === "BKE00026") {
              this.dialogService.customMessage('error', "common.message.phone-duplicate");
            }
  ...
  public validatorOtp(): boolean {
    this.changePhoneForm.get('otp')?.addValidators([this.utils.requireValidator,  Validators.maxLength(6), this.utils.validateFaxPhonePostCode]);
  ...
  public updatePhone() {
    this.loadingDialog.showSpinner(true);
    this.customerService.updatePhoneCustomer({phone: this.changePhoneForm.get('phone')?.value, codePhone: this.changePhoneForm.get('codePhone')?.value}).subscribe({
      next: (res: HttpClientResponse) => {
        console.log(res);
        this.loadingDialog.showSpinner(false);
        if (res.meta.code === "200") {
          this.dialogRef.close(res);
          this.toaster.show('success', 'Success', 'Your phone number has been changed.');
        }
      },
      error: (err: HttpErrorResponse) => {
        this.loadingDialog.showSpinner(false);
        this.toaster.show('error', 'Error', "Can't changed your phone number.");
      }
    });
  }
  // ----------- action step----------------
