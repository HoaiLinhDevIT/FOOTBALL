...
export class ChangeEmailComponent implements OnInit {
  ...
  public onNoClick(): void {
    if (this.changeEmailForm.get('email')?.value !== this.data) {
  ...
  public stepEmail() {
    if (!this.validatorEmail()) {
      ...
      this.exporterService.checkMail(
        {
        ...
        .subscribe({
          ...
          error: (err: HttpErrorResponse) => {
            ...
            if (err.error.meta.code === "BKE00026") {
              this.dialogService.customMessage('error', "screen.profile.message.email-same-old-email");
            }
  ...
  public updateEmail() {
    this.loadingDialog.showSpinner(true);
    this.exporterService.updateEmailExporter({ email: this.changeEmailForm.get('email')?.value }).subscribe({
      next: (res: HttpClientResponse) => {
        this.loadingDialog.showSpinner(false);
